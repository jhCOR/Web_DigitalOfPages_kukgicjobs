...
module.exports = new LocalStrategy({
	    ...
	    process.nextTick(function() {
			...
			database.AdminUserModel.findOne({ 'name' :  paramName }, function(err, user) {
		    ...
		    database.AdminUserModel.findOne({ 'email' :  email }, function(err, user) {
		        // 에러 발생 시
		        if (err) {
		            return done(err);
		        }
		        ...
		        if (user) {
		        	...
		        	// 모델 인스턴스 객체 만들어 저장
		        	var user = new database.AdminUserModel({'email':email, 'password':password, 'name':paramName, 'group':paramGroup});
		        	user.save(function(err) {
		        		if (err) {
		        			throw err;
		        		}
		        		
		        	    console.log("관리자 데이터 추가함.");
