    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/uploader'; 	//upload URL
    	...
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		...
    		try{
	    		if(!!tempFile){
	    			//Ajaxs통신하는 부분. 파일과 업로더할 url을 전달한다.
	...
	function callAjaxForHTML5 (tempFile, sUploadURL){
			alert(tempFile.content+"/"+tempFile.name+"/");
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
	...
	var setPhotoToEditor2 = function (req, res) {
		console.log('setPhotoToEditor2 호출됨'+req);
	if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			//스마트 에디터 플러그인을 통해서 넣는 방법 (oFileInfo는 Array)
			opener.nhn.husky.PopUpManager.setCallback(window, 'SET_PHOTO', [oFileInfo]);
			//본문에 바로 tag를 넣는 방법 (oFileInfo는 String으로 <img src=....> )
	 ...
	 module.exports.setPhotoToEditor2= setPhotoToEditor2;